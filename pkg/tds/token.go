...
func writeValue(buf *bytes.Buffer, val interface{}, col Column) {
	...
	case TypeDecimalN, TypeNumericN:
		writeDecimalValueFromInterface(buf, val, col.Precision, col.Scale)
...
// writeDecimalValueFromInterface writes a decimal/numeric value from any supported type.
func writeDecimalValueFromInterface(buf *bytes.Buffer, val interface{}, precision, scale uint8) {
	...
	writeDecimalValue(buf, d, precision, scale)
}

// writeDecimalValue writes a decimal.Decimal value.
func writeDecimalValue(buf *bytes.Buffer, d decimal.Decimal, precision, scale uint8) {
	...
	// Get the coefficient as a big.Int, then convert to uint64
	// For values that fit in uint64
	coef := scaled.Coefficient()
	var val uint64
	if coef.IsUint64() {
		val = coef.Uint64()
	} else if coef.IsInt64() {
		val = uint64(coef.Int64())
	} else {
		// Value too large - truncate (would need big int support for larger)
		val = coef.Uint64()
	}

	// Calculate byte length needed based on precision
